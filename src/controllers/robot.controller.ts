...
export async function initiateBot() {
  ...
  const trainingData = JSON.parse(fs.readFileSync('src/training_data/ntachi.json', 'utf8'));
...
export async function demo(userInput: string) {
  // Process user input
  const response = await nlp.process('en', userInput);

  if (!response || response.intent === 'None') {
    // If no intent was detected or confidence score is low, use the fallback intent
